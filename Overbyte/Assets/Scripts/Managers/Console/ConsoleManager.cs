...
public class ConsoleManager : MonoBehaviour
{
    ...
    private InputAction autoConfirm;
    ...
    private void Awake()
    {
        ...
        toggleConsole = playerInput.actions["DEVConsole"];
        navigate = playerInput.actions["Navigate"];
        autoConfirm = playerInput.actions["Submit"];

        toggleConsole.performed += ctx => ToggleConsole();
        navigate.performed += OnNavigate;
        autoConfirm.performed += ctx => ConfirmSelection();
        // historyUp.performed += ctx => NavigateHistoryUp();
        // historyDown.performed += ctx => NavigateHistoryDown();
    }
    ...
    private void HandleNewLog(string message, LogType type)
    {
        ...
        switch (type)
        {
            ...
            case LogType.InputManager: titleTMP.color = new Color(1f, 0.4f, 0.7f); break;
            case LogType.Localitzation: titleTMP.color = new Color(1f, 0.65f, 0f); break;
    ...
    private void ConfirmSelection()
    {
        ...
        {
            HandleInputSubmit(consoleInput.text);
        }
    }

    private void NavigateHistoryUp()
    {
        if (!consoleInput.isFocused || autoCompletePanel.activeSelf) return;
