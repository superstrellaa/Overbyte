...
public class MapManager : Singleton<MapManager>
{
    ...
    [SerializeField] private GameObject shootGivenPrefab;
    [SerializeField] private List<GameObject> weaponPrefabs;
    [SerializeField] private string gunContainerName = "GunContainer";
    [SerializeField] private List<WeaponConfigSO> weaponConfigs;
    ...
    private Dictionary<string, float> remoteTargetPitch = new Dictionary<string, float>();
    ...
    {
        sceneReady = sceneName.StartsWith("sc_TestRoom");
        if (sceneReady)
        {
    ...
    private void InstantiateRemotePlayer(string uuid, Vector3 pos, Quaternion rot, Scene targetScene)
    {
        ...
        var networkId = other.AddComponent<PlayerNetworkId>();
        networkId.SetUuid(uuid);
    ...
    private void Update()
    {
        ...
        foreach (var kvp in remotePlayers)
        {
            ...
            if (remotePlayer == null || !remoteTargetPitch.ContainsKey(uuid))
                continue;

            Transform gunContainer = remotePlayer.transform.Find("Player")?.Find(gunContainerName);
            if (gunContainer == null || gunContainer.childCount == 0)
                continue;

            Transform gunTransform = gunContainer.GetChild(0);
            float targetPitch = remoteTargetPitch[uuid];
            Quaternion targetRot = Quaternion.Euler(targetPitch, 0f, 0f);

            gunTransform.localRotation = Quaternion.Lerp(gunTransform.localRotation, targetRot, Time.deltaTime * 10f);
        }
    ...
    public void RemoveRemotePlayer(string uuid)
    {
        ...
        LogManager.LogDebugOnly($"Remote player {uuid} removed.", LogType.Gameplay);
    }
    ...
    public void SetRemotePlayerGun(string uuid, string gunName)
    {
        if (!remotePlayers.TryGetValue(uuid, out var remotePlayer) || remotePlayer == null)
        {
            LogManager.LogDebugOnly($"SetRemotePlayerGun: no remote player found for {uuid}", LogType.Warning);
            return;
        }
        ...
        {
            LogManager.LogDebugOnly($"SetRemotePlayerGun: GunContainer not found on {uuid}", LogType.Warning);
            return;
        }
        ...
        GameObject prefab = weaponPrefabs.Find(w => w.name == gunName);
        if (prefab == null)
        {
            LogManager.LogDebugOnly($"SetRemotePlayerGun: prefab '{gunName}' not found in weaponPrefabs", LogType.Warning);
            return;
        }

        GameObject newGun = GameObject.Instantiate(prefab, gunContainer);
        newGun.transform.localPosition = Vector3.zero;
        newGun.transform.localRotation = Quaternion.identity;

        LogManager.LogDebugOnly($"Remote player {uuid} equipped gun {gunName}", LogType.Gameplay);
    }

    public void SetRemotePlayerAiming(string uuid, float pitch)
    {
        remoteTargetPitch[uuid] = -pitch; 
    }

    public void RemotePlayerShoot(string uuid, string hit, Vector3 hitPoint, Vector3 hitNormal, string gun)
    {
        if (!remotePlayers.TryGetValue(uuid, out var remotePlayer) || remotePlayer == null)
            return;
        ...
        Transform currentWeapon = gunContainer.GetChild(0);
        Transform particleContainer = currentWeapon.Find("ParticleContainer");
        WeaponConfigSO config = weaponConfigs.Find(w => w.weaponName == gun);

        if (particleContainer != null && config != null && config.muzzlePrefab != null)
        {
            var parent = gun == "Bombard" ? null : particleContainer;

            var muzzleInstance = Instantiate(config.muzzlePrefab, particleContainer.position, Quaternion.identity, parent);
            ...
            if (gun == "Bombard")
            {
                ...
                if (hit != "none")
                {
                    Vector3 aimDir = (hitPoint - muzzleInstance.transform.position).normalized;
        ...
        if (config != null && config.bulletHolePrefab != null && hit == "wall")
        {
            StartCoroutine(SpawnBulletHoleWithDelay(config, hitPoint, hitNormal));
        }
    }

    private IEnumerator SpawnBulletHoleWithDelay(WeaponConfigSO config, Vector3 position, Vector3 normal)
    {
        yield return new WaitForSeconds(config.bulletHoleDelay);

        Quaternion rotation = Quaternion.LookRotation(normal);
        Instantiate(config.bulletHolePrefab, position + normal * 0.001f, rotation);
    }

    public void RemotePlayerShootGiven(string targetUuid, int damage)
    {
        if (!remotePlayers.TryGetValue(targetUuid, out var remotePlayer) || remotePlayer == null || 
            !ConfigManager.Instance.Data.showDamageNumbers)
            return;

        Transform playerTransform = remotePlayer.transform.Find("Player");
        if (playerTransform == null)
            playerTransform = remotePlayer.transform;

        Vector3 spawnPos = playerTransform.position + Vector3.up * 2f;

        GameObject damagePopup = Instantiate(shootGivenPrefab, spawnPos, Quaternion.identity);
        damagePopup.transform.SetParent(null);

        var damageText = damagePopup.transform.Find("Damage")?.GetComponent<TMPro.TMP_Text>();
        if (damageText != null)
        {
            damageText.text = damage.ToString();
        }

        var popup = damagePopup.GetComponent<DamagePopup>();

        popup.Initialize(Camera.main);
    }
}
