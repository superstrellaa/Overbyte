        ...
        socket.OnClose += (code) =>
        {
            ...
            else
            {
                ...
                if (PlayerManager.Instance != null && PlayerManager.IsAlive && PlayerManager.Instance.CurrentState != PlayerState.Loading)
                {
                    ...
                    var escManager = ESCManager.Instance;

                    playerManager.SetState(PlayerState.Loading);
                    sceneTransitionManager.TransitionTo("sc_LoadingGame", withFade: false);

                    guiManager.SetGUIOpen(true);

                    var playerObj = playerManager.gameObject.transform.Find("Player").gameObject;
                    ...
                    popupManager.Open(PopupType.ConnectionRefused);
                    guiManager.ShowPanel(guiManager.lobbyMatchFound, false);
                    escManager.CloseESCMenuCursor();
                }
            }
        };
